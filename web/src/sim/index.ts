...
export interface FacilityRenderRecord {
	x: number;
	y: number;
	width: number;
	tileType: string;
...
export interface StepResult {
	...
	notifications: Array<{ kind: string; message: string; soundResourceId?: number }>;
	prompts: Array<{
		promptId: string;
...
export class TowerSim {
	...
	step(): StepResult {
		...
		if (
			this.ledger.cashBalance !== balanceBefore &&
			this.world.gateFlags.newspaperTrigger !== 0
		) {
			notifications.push({ kind: "sound", soundResourceId: 0x271d });
		}
		...
		return {
			...
			notifications: notifications.map((n) => ({
				kind: n.kind,
				message: n.message ?? "",
				...(n.soundResourceId !== undefined
					? { soundResourceId: n.soundResourceId }
					: {}),
			})),
			prompts,
		};
	...
	setNewsViewport(left: number, top: number, width: number, height: number): void {
		this.world.eventState.newsViewport = {
			left: Math.max(0, Math.trunc(left)),
	...
	facilitiesToArray(): FacilityRenderRecord[] {
		return Object.entries(this.world.placedObjects)
			.map(([key, record]) => {
				const [x, y] = key.split(",").map(Number);
				const sidecar =
					record.linkedRecordIndex >= 0
						? this.world.sidecars[record.linkedRecordIndex]
						: undefined;
				const visualState =
					sidecar?.kind === "entertainment_link"
						? sidecar.linkPhaseState
						: sidecar?.kind === "commercial_venue"
							? sidecar.availabilityState
							: record.unitStatus;
				return {
					x,
					y,
					width: record.rightTileIndex - record.leftTileIndex + 1,
					tileType: this.world.cells[key] ?? "empty",
					objectTypeCode: record.objectTypeCode,
					unitStatus: record.unitStatus,
					evalLevel: record.evalLevel,
					visualState,
	...
	get eventPresentation(): {
		...
		return {
			gameStateFlags: this.world.eventState.gameStateFlags,
			fireFloor: this.world.eventState.fireFloor,
			fireTile: this.world.eventState.fireTile,
			helicopterExtinguishPos:
				this.world.eventState.helicopterExtinguishPos,
		};
	}
