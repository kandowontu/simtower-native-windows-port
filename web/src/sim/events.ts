...
// ─── Tuning constants ────────────────────────────────────────────────────────
// From custom tuning resource PART #1000 at DS:0xe6xx.
...
const BOMB_RANSOM: Record<number, number> = {
	2: ORIGINAL_TUNING.events.bombRansomByStars2To4[0],
	3: ORIGINAL_TUNING.events.bombRansomByStars2To4[1],
	4: ORIGINAL_TUNING.events.bombRansomByStars2To4[2],
};
/** Blast rectangle dimensions. */
const BLAST_HALF_TILES = 20;

const FIRE_SPREAD_RATE = ORIGINAL_TUNING.events.fireSpreadRate; // DS:0xe644
const FIRE_VERTICAL_DELAY = ORIGINAL_TUNING.events.fireVerticalDelay; // DS:0xe646
const HELICOPTER_EXTINGUISH_RATE = ORIGINAL_TUNING.events.helicopterExtinguishRate; // DS:0xe648
const HELICOPTER_PROMPT_DELAY = ORIGINAL_TUNING.events.helicopterPromptDelay; // DS:0xe64a
const RESCUE_COUNTDOWN_WITH_SECURITY = ORIGINAL_TUNING.events.rescueCountdownWithSecurity; // DS:0xe64c
const HELICOPTER_RESCUE_COST = ORIGINAL_TUNING.events.helicopterRescueCost; // DS:0xe688
...
// Random-news event: the binary drives audio playback only. The classifier
// feeds play_classified_news_sound (11d0:042c); there is no visual popup.
...
function newsRecordAt(
	world: WorldState,
	x: number,
	y: number,
): { record: WorldState["placedObjects"][string]; sidecar: WorldState["sidecars"][number] | undefined } | null {
	const key = `${x},${y}`;
	if (!world.cells[key]) return null;
	const anchor = world.cellToAnchor[key] ?? key;
	const record = world.placedObjects[anchor];
	if (!record) return null;
	return {
		record,
		sidecar:
			record.linkedRecordIndex >= 0
				? world.sidecars[record.linkedRecordIndex]
				: undefined,
	};
}

/** classify_news_slot_subject @ 11d0:06c0. */
export function classifyNewsSlotSubject(
	world: WorldState,
	x: number,
	y: number,
): number {
	const found = newsRecordAt(world, x, y);
	if (!found) return yToFloor(y) < 10 ? -2 : -1;
	const { record, sidecar } = found;
	const family = record.objectTypeCode;
	const phase = record.unitStatus & 7;
	if ([3, 4, 5, 9].includes(family)) {
		return record.unitStatus < 0x10 && phase !== 0 ? family : -2;
	}
	if (family === 7) {
		return record.unitStatus < 8 && phase !== 0 ? family : -2;
	}
	if ([6, 10, 12, 16].includes(family)) {
		if (sidecar?.kind !== "commercial_venue") return -2;
		if ([3, 0xff].includes(sidecar.availabilityState)) return -2;
		return sidecar.currentPopulation !== 0 || sidecar.todayVisitCount !== 0
			? family
			: -2;
	}
	if ([24, 44].includes(family)) return record.unitStatus > 1 ? 0x0b : -2;
	if ([29, 30].includes(family)) {
		return sidecar?.kind === "entertainment_link" && sidecar.linkPhaseState > 1
			? 0x1d
			: -2;
	}
	// Paired cinema subjects reach an unmapped 0x2329+selector code and are
	// silent in the supplied binary because no matching wave exists.
	if ([18, 19, 34, 35].includes(family)) {
		return sidecar?.kind === "entertainment_link" && sidecar.linkPhaseState === 3
			? 0x2329 + sidecar.familySelectorOrSingleLinkFlag
			: -2;
	}
	return -2;
}

/** play_classified_news_sound @ 11d0:042c, returning the original WAVE ID. */
export function selectNewsSoundForSubject(
	world: WorldState,
	time: TimeState,
	subject: number,
): number | null {
	if ([3, 4, 5].includes(subject)) return 0x629;
	if (subject === 6) return sampleRng(world) % 2 === 0 ? 0x568 : 0x569;
	if (subject === 7) return 0x5a8;
	if (subject === 9) return sampleRng(world) % 10 === 0 ? 0x628 : 0x629;
	if (subject === 0x0b) return sampleRng(world) % 2 === 0 ? 0x6a8 : 0x6a9;
	if ([10, 12, 16].includes(subject)) {
		return sampleRng(world) % 2 === 0 ? 0x569 : 0x668;
	}
	if ([0x1d, 0x1e].includes(subject)) return 0x0b28;
	if (subject === -1) {
		const calendarQuarter = Math.floor(time.dayCounter / 3) % 4;
		if (calendarQuarter === 2 && time.daypartIndex < 4) return 0x271c;
		if (calendarQuarter === 3 && time.daypartIndex >= 4) return 0x271b;
	}
	return null;
}
...
export function triggerRandomNewsEvent(
	...
	const viewport = es.newsViewport ?? {
		left: 0,
		top: 0,
		width: world.width,
		height: world.height,
	};
	const width = Math.max(1, Math.min(viewport.width, world.width - viewport.left));
	const height = Math.max(1, Math.min(viewport.height, world.height - viewport.top));
	const bucket = sampleRng(world) % 6;
	const quarterX = Math.floor(width / 4);
	const relativeX = [quarterX, Math.floor(width / 2), width - quarterX - 1][
		bucket % 3
	];
	const lastRow = height - 1;
	const relativeY =
		bucket < 3
			? Math.floor(lastRow / 2)
			: lastRow - Math.floor(lastRow / 4);
	const x = Math.max(0, Math.min(world.width - 1, viewport.left + relativeX));
	const y = Math.max(0, Math.min(world.height - 1, viewport.top + relativeY));
	const subject = classifyNewsSlotSubject(world, x, y);
	const soundResourceId = selectNewsSoundForSubject(world, time, subject);
	if (soundResourceId !== null) {
		world.pendingNotifications.push({ kind: "sound", soundResourceId });
	}
}
