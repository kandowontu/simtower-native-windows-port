...
function placeCathedralStack(
	...
	if (!deferRebuild) runGlobalRebuilds(world, ledger);
...
function placeRecyclingCenterStack(
	...
	if (
		!allowLegacyPlacement &&
		!hasRecyclingStackOverlap(world, yToFloor(upperY))
	) {
		return {
			accepted: false,
			reason:
...
function partyHallSubRecords(
	...
	return [
		{ offsetX: x, rowY: upperY, width: 24, familyCode: FAMILY_PARTY_HALL },
		{
			offsetX: x,
			rowY: lowerY,
			width: 24,
...
export function handlePlaceTile(
	...
	if (normalizedTileType === "parkingRamp") {
		...
		if (!allowLegacyPlacement && !isTopOfStack && !hasRampAbove) {
			return {
				accepted: false,
				reason:
